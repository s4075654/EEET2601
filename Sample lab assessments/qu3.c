...
void NicelyDisplayAPattern(
    unsigned short int an_integer_between_1_and_15_inclusively) {

  printf(" ");
  for (unsigned short int row1 = 1;
       row1 <= an_integer_between_1_and_15_inclusively; row1++)
    printf("%-3hu", row1);
  for (unsigned short int column = 2;
       column <= an_integer_between_1_and_15_inclusively; column++) {
    printf("\n ");
    for (unsigned short int row = column; row >= 1; row--)
      printf("%-3hu", row);
    DisplayTheSpaces(an_integer_between_1_and_15_inclusively - column);
    if (an_integer_between_1_and_15_inclusively - column != 0)
      printf("%-3hu", an_integer_between_1_and_15_inclusively - column + 1);
  }
}
