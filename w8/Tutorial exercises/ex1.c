...
unsigned short int ReturnTheMaximumElementInThatArray(int array[], int size) {
    ...
    for (int i = 0; i < size; i++) {
        if (maximum_element < array[i]) maximum_element = array[i];
    }

    return(maximum_element);
}

unsigned short int Return1IfTheIntegerIsFoundInTheArrayAnd0Otherwise(int array[], int size, int another_integer) {
    ...
    for (int i = 0; i < size; i++) {
        if (another_integer == array[i]) return(1);
    }
...
int main() {
    ...
    printf("%hu", Return1IfTheIntegerIsFoundInTheArrayAnd0Otherwise(arr, SIZE, another_integer));
