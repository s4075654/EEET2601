...
long double CalculateTheAverageOfElementsWhichAreGreaterThanOrEqualTo50 (int* array) {
    long long int the_sum_of_elements = 0;
    unsigned long long int number_of_elements_which_are_greater_than_or_equal_to_50 = 0;
    
    for (unsigned long long int element = 0; element < sizeof(arr) / 4; ++element) {
        if (*array >= 50) {
    ...
    return((long double)the_sum_of_elements / (long double)number_of_elements_which_are_greater_than_or_equal_to_50);
}
