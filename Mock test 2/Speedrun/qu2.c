...
long double SumOfElementsGreaterThanOrEqualTo50(int *array, int size) {
  if (size == 0)
    return (0);
  if (*array >= 50)
    return (*array + SumOfElementsGreaterThanOrEqualTo50(array + 1, size - 1));
  else
    return (SumOfElementsGreaterThanOrEqualTo50(array + 1, size - 1));
}
...
int main() {
  ...
  while (times < size) {
    if (*array >= 50)
      number_of_elements_greater_than_or_equal_to_50++;
    array++;
    times++;
  }
  ...
  printf("The average of elements which are greater than or equal to 50: %Lf\n",
         SumOfElementsGreaterThanOrEqualTo50(array, size) /
             number_of_elements_greater_than_or_equal_to_50);
  ...
  printf("The closest element with the inputted integer within the array: %d",
         arr[index]);
