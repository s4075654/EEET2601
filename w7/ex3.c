...
char* ReplaceTheLowercaseCharactersByUppercaseAndViceVersa(char* a_string) {
    ...
    for (unsigned long long int character = 0; character < sizeof(a_string); ++character) {
...
void PrintOutTheStringAfterReplacement(char* another_string, char* a_searching_keyword, char* text_for_replacement) {
    ...
    if (strstr(another_string, a_searching_keyword) != NULL) {
        ...
        for (unsigned long long int character = strstr(another_string, a_searching_keyword) - another_string; character < sizeof(a_searching_keyword) - 1; ++character) {
            character[another_string] = *character_for_replacement;
            ++character_for_replacement;
        }
        return(PrintOutTheStringAfterReplacement(another_string, a_searching_keyword, text_for_replacement));
    }
    else {
...
int main() {
    ...
    printf("Replaced the lowercase characters by uppercase and vice versa: %s", ReplaceTheLowercaseCharactersByUppercaseAndViceVersa(AskTheUserToInputAString()));
    char* another_string = AskTheUserToEnterAnotherString();
    text find_text = ASearchingKeyword();
    PrintOutTheStringAfterReplacement(another_string, find_text.a_searching_keyword, TextForReplacement(find_text));
