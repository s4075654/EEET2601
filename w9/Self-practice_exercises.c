...
long long int TheSumOfAllIntegers(long long int n) {
    ...
    if (n == 1) return(the_sum_of_all_integers + n);
...
size_t TheSumOfDigitsOfAPositiveInteger(size_t n) {
    size_t the_sum_of_digits_of_a_positive_integer = 0, positive_integer = n, digits = 0;
...
void PrintOutTheDigitsOfAPositiveInteger(size_t n) {
    ...
    for (unsigned long long int character = 0; character < digits; character++) printf("%c ", digits_of_a_positive_integer[character]);
}

void PrintOutAllCharactersOfAStringUsingRecursion(char* a_string, unsigned long long int character) {

    if (character == strlen(a_string)) return;
