...
void PrintOutTheFirstNamesInOneLine(char **full_names) {
  ...
  for (unsigned char first_names = 0; first_names < 4; ++first_names) {
    ...
    while (full_names[first_names][character] > 32 &&
           full_names[first_names][character] < 127) {
...
void TheFullNames(char **full_names) {
  ...
  if (strcmp(full_names[0], full_names[1]) < 0 &&
      strcmp(full_names[1], full_names[2]) < 0 &&
      strcmp(full_names[2], full_names[3]) < 0) {
