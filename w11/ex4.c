...
long long int ReturnItsAverageValue(long long int *an_array,
                                    unsigned long long int size) {
  long long int values = 0;

  for (unsigned long long int value = 0; value < size; value++)
    values += an_array[value];
...
size_t ReturnSumOfEvenDigitsInTheString(char *a_string) {
  ...
  for (unsigned long long int digit = 0; a_string[digit] != '\0'; digit++)
    if (((a_string[digit] - '0') % 2 == 0) && (a_string[digit] >= 48) &&
        (a_string[digit] <= 57))
      sum_of_even_digits_in_the_string += (a_string[digit] - '0');
...
void PrintOutEachWord(char *a_string) {
  ...
  for (unsigned long long int character = 0; a_string[character] != '\0';
       character++) {
...
void PrintOutTheLongestWordOfThatString(char *a_string,
                                        unsigned long long int length) {
  unsigned long long int count = 0, beginning = 0, true_beginning = 0,
                         max_count = 0;

  for (unsigned long long int character = 0; character <= length; character++) {
    if ((a_string[character] != ' ') && (a_string[character] != '\0')) {
      count++;
      if ((a_string[character] != '\0') && (a_string[character] != ' ') &&
          (a_string[character - 1] == ' '))
        beginning = character;
    } else {
      if (max_count < count) {
        max_count = count;
  ...
  for (unsigned long long int character = true_beginning;
       (a_string[character] != ' ') && (a_string[character] != '\0');
       character++)
    printf("%c", a_string[character]);
}
