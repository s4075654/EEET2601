...
long double PrintOutTheirDifference(time time_value_one, time time_value_two) {
    long double first_time = time_value_one.hour + (long double)time_value_one.minute / 60 + (long double)time_value_one.second / 3600;
    long double second_time = time_value_two.hour + (long double)time_value_two.minute / 60 + (long double)time_value_two.second / 3600;
...
long double PrintOutTheLatestTime(time* an_array_of_4_time_values) {
    ...
    for (char time = 0; time < 4; time++) times[time] = an_array_of_4_time_values[time].hour + (long double)an_array_of_4_time_values[time].minute / 60 + (long double)an_array_of_4_time_values[time].second / 3600;
    
    long double latest_time = times[0];

    for (char value = 0; value < 4; value++) if (latest_time < times[value]) latest_time = times[value];
    
    return(latest_time);
}

void PrintOutAllTimeValuesOfTheArrayWhichHaveThatHourValue(time* an_array_of_4_time_values) {
    long long int hour_value;

    printf("Input an hour value: ");
    scanf("%lld", &hour_value);
    for (char time_value = 0; time_value < 4; time_value++) {
        if ((an_array_of_4_time_values[time_value].hour == hour_value) || (an_array_of_4_time_values[time_value].minute == hour_value) || (an_array_of_4_time_values[time_value].second == hour_value)) {
            printf("%lld:%lld:%lld", an_array_of_4_time_values[time_value].hour, an_array_of_4_time_values[time_value].minute, an_array_of_4_time_values[time_value].second);
        }
    }
}
...
int main() {
    ...
    PrintOutAllTimeValuesOfTheArrayWhichHaveThatHourValue(an_array_of_4_time_values);
